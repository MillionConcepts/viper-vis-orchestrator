...
def dump_objects(session=None):
    ...
    if len(images) < 530:
        ...
        for i in range(530 - len(images)):
            offset_record = copy_imagerecord(
                random.choice(images), copy_files=True
            )
...
# make 40 random RequestForms and use them to create ImageRequests in the db
for _ in range(40):
...
# randomly select 1-2 ImageRecords to assign to half of these ImageRequests.
# VIS-verify records for the first 10 requests, and no others.
with OSession() as session:
    our_records = []
    ...
    for req_ix in range(len(requests) // 2):
        ...
        for rec in records:
            ...
            # verify if in the first 5 requests
            if req_ix <= 5:
        ...
        # now evaluate the request if it is in the first 3 requests.
        if req_ix > 3:
            continue
...
print(f"assigned {len(our_records)} records to {len(requests) // 2} requests")

products_by_compression = image_records_by_compression()
# dump any lossless products
with OSession() as session:
...
lossies = []
# we've made a bunch of random ImageRecords, so this might fail; lazily keep
# trying until we get some undeleted ones
while len(lossies) < 5:
    pick = random.choice(all_lossies)
    try:
        # don't commit the last one, just check it can be made -- this is to
        # test functionality a little further down
...
del pick
print(
    f"made protected list entries for " f"{', '.join(l._pid for l in lossies)}"
)
...
with OSession() as session:
    ...
    session.commit()
    assert (
        ProtectedListEntry.from_pid(lossies[0].product_id).has_lossless is True
    )
    print(f"checked PL response to lossless downlink of {lossies[0]._pid}")
    # make a different lossless product, from the selected lossy product we did
    # _not_ make a PL request for
